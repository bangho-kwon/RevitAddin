...
{
    public static class DuctFittingConnectorExtractor
    {
        public static List<UnifiedInfo> Extract(Document doc)
        {
            ...
            foreach (var fi in fittings)
            {
                ...
                string systemType = "";
                var sysId = fi.get_Parameter(BuiltInParameter.RBS_DUCT_SYSTEM_TYPE_PARAM)?.AsElementId() ?? ElementId.InvalidElementId;
                ...
                if (connectorList.Count == 1 && IsCap(partType))
                {
                    ...
                    string diameter = GetFormattedConnectorSize(doc, conn, out string width, out string height);
                ...
                foreach (var conn in connectorList)
                {
                    if (conn == null || conn.Domain != Domain.DomainHvac) continue;
        ...
        private static string GetFormattedConnectorSize(Document doc, Connector conn, out string width, out string height)
        {
            width = "";
            height = "";
            ...
            {
                double dia = conn.Radius * 2;
                var opts = doc.GetUnits().GetFormatOptions(SpecTypeId.DuctSize);
                var unitTypeId = opts.GetUnitTypeId();
                double converted = UnitUtils.ConvertFromInternalUnits(dia, unitTypeId);
