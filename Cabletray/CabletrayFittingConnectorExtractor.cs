...
{
    public static class CabletrayFittingConnectorExtractor
    {
        public static List<UnifiedInfo> Extract(Document doc)
        {
            ...
            foreach (var fi in fittings)
            {
                ...
                if (connectorList.Count == 2 && IsTransition(partType))
                {
                    ...
                    string connectorLength = (distMM / 2).ToString("0.##");

                    foreach (var conn in connectorList)
                    {
                        string width = GetFormattedWidth(doc, conn);
                        string height = GetFormattedHeight(doc, conn);
                ...
                foreach (var conn in connectorList)
                {
                    if (conn == null || conn.Domain != Domain.DomainCableTrayConduit) continue;
        ...
        private static string GetFormattedWidth(Document doc, Connector conn)
        {
            if (conn == null) return "";
            var opts = doc.GetUnits().GetFormatOptions(SpecTypeId.CableTraySize);
            var unitTypeId = opts.GetUnitTypeId();
            double converted = UnitUtils.ConvertFromInternalUnits(conn.Width, unitTypeId);
            return converted.ToString("0.##", CultureInfo.InvariantCulture);
        }

        private static string GetFormattedHeight(Document doc, Connector conn)
        {
            if (conn == null) return "";
            var opts = doc.GetUnits().GetFormatOptions(SpecTypeId.CableTraySize);
            var unitTypeId = opts.GetUnitTypeId();
            double converted = UnitUtils.ConvertFromInternalUnits(conn.Height, unitTypeId);
            return converted.ToString("0.##", CultureInfo.InvariantCulture);
        }
        ...
        private static bool IsTransition(string partType)
        {
            ...
            partType = partType.ToLower();
            return partType.Contains("transition") ||
                   partType.Contains("channelcabletraytransition") ||
                   partType.Contains("laddercabletraytransition");
        }
    }
}
