...
{
    public static class PipeAccyConnectorExtractor
    {
        public static List<UnifiedInfo> Extract(Document doc)
        {
            ...
            foreach (var fi in fittings)
            {
                ...
                // BasicSize: RBS_CALCULATED_SIZE → 포맷 적용
        ...
        private static string GetFormattedDiameter(Document doc, Connector conn)
        {
            ...
            double diameter = conn.Radius * 2; // 내부 단위 (feet)

            // Pipe Size 단위 기준 (mm, inch 등)
            ...
            return converted.ToString("0.##", CultureInfo.InvariantCulture); // 단위 제거
        }
